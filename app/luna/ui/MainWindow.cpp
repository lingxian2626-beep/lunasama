...
MainWindow::MainWindow(QWidget* parent) : QWidget(parent) {
  ...
  io_->setNames(QString::fromUtf8("主人"), QString::fromUtf8("露娜"));
  ...
  // 提醒轮询定时器：每 90 秒查一次待办任务
  reminderTimer_ = new QTimer(this);
  reminderTimer_->setInterval(90'000);
  connect(reminderTimer_, &QTimer::timeout, this, [this]{
    backend_->fetchReminders();
  });
...
void MainWindow::connectSignals() {
  ...
  backend_->setTextLang(QStringLiteral("zh"));

  // 提醒：有未完成任务时，露娜主动播报（对话中/已淡出时不打扰）
  connect(backend_, &BackendClient::remindersReady, this, [this](int count, const QString& text){
    if (count <= 0) return;
    if (reminderBusy_) return;          // 正在播报/对话中不打扰
    if (gateTimer_ && gateTimer_->isActive()) return;  // 对话进行中不打扰
    reminderBusy_ = true;
    io_->showOutput(text);
    backend_->speakReminder(text);
  });

  // 提醒音频就绪 → 播放；播完恢复输入
  connect(backend_, &BackendClient::reminderAudioReady, this, [this](const QUrl& audio){
    audio_->play(audio);
  });
  ...
  // TTS 失败等情况下兜底恢复
  connect(backend_, &BackendClient::reminderSpoken, this, [this]{
...
void MainWindow::showContextMenu(const QPoint& globalPos) {
  QMenu menu;
  auto* modesMenu = menu.addMenu("更换服装");
  populateModesMenu(modesMenu);

  auto* dragMenu  = menu.addMenu("拖拽按键");
  populateDragBindingMenu(dragMenu);

  menu.addSeparator();
  menu.addAction("关闭", []{ qApp->quit(); });
  menu.exec(globalPos);
}
...
void MainWindow::populateModesMenu(QMenu* menu) {
  ...
  if (names.isEmpty()) {
    auto* a = menu->addAction("未找到服装（ui/assets/modes/<文件夹>/png）");
    a->setEnabled(false);
    return;
  }
